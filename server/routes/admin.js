...
router.get('/dashboard', async (req, res) => {
  try {
    ...
    const totalBetCoins = transactions.filter(t => t.type === 'bet').reduce((s, t) => s + Math.abs(t.coins), 0)
    const totalWinCoins = transactions.filter(t => t.type === 'win').reduce((s, t) => s + t.coins, 0)
    const houseProfit = totalBetCoins - totalWinCoins

    const since = new Date(Date.now() - 24 * 60 * 60 * 1000)
    const activeUsers = await Round.distinct('userId', { createdAt: { $gte: since } })
    ...
    const betTypes = await Round.aggregate([
      { $group: { _id: '$betType', count: { $sum: 1 }, totalWagered: { $sum: '$betAmount' } } },
    ])

    res.json({
      totalUsers, totalRounds, bannedUsers, flaggedUsers,
      activeUsers: activeUsers.length,
      totalDeposits, totalWithdrawals, houseProfit,
      netRevenue: totalDeposits - totalWithdrawals,
      pendingWithdrawals,
      dailyRevenue, recentRounds, betTypes,
    })
  } catch (err) {
    console.error(err)
...
router.get('/users/:id', async (req, res) => {
  try {
    ...
    const rounds = await Round.find({ userId: req.params.id }).sort({ createdAt: -1 }).limit(20)
    const txns = await Transaction.find({ userId: req.params.id }).sort({ createdAt: -1 }).limit(20)
...
router.post('/users/:id/ban', async (req, res) => {
  try {
    ...
    await log(req.user.id, req.user.username, 'ban_user', user._id, user.username, reason)
...
router.post('/users/:id/unban', async (req, res) => {
  try {
    ...
    await log(req.user.id, req.user.username, 'unban_user', user._id, user.username, '')
...
router.post('/users/:id/reset-balance', async (req, res) => {
  try {
    ...
    await log(req.user.id, req.user.username, 'reset_balance', user._id, user.username, `Set to ${coins} coins`)
    res.json({ message: `Balance reset to ${user.coins} coins`, coins: user.coins })
  } catch (err) {
    res.status(500).json({ message: 'Server error' })
  }
})
...
router.post('/users/:id/add-coins', async (req, res) => {
  try {
    ...
    await Transaction.create({
      ...
      type: 'bonus', amount: 0, coins,
      note: reason || `Admin added ${coins} coins`,
    })
    await log(req.user.id, req.user.username, 'add_coins', user._id, user.username, `+${coins} coins: ${reason}`)
    res.json({ message: `Added ${coins} coins to ${user.username}`, coins: user.coins })
  } catch (err) {
    res.status(500).json({ message: 'Server error' })
  }
})
...
router.post('/users/:id/flag', async (req, res) => {
  try {
    ...
    const user = await User.findByIdAndUpdate(req.params.id, { isFlagged: flagged }, { new: true })
    await log(req.user.id, req.user.username, flagged ? 'flag_user' : 'unflag_user', user._id, user.username, '')
    res.json({ message: `${user.username} ${flagged ? 'flagged' : 'unflagged'}` })
  } catch (err) {
    res.status(500).json({ message: 'Server error' })
  }
})
...
router.post('/withdrawals/:id/approve', async (req, res) => {
  try {
    ...
    await log(req.user.id, req.user.username, 'approve_withdrawal', txn.userId, txn.username,
      `$${txn.amount} withdrawal approved`)
    res.json({ message: `Withdrawal of $${txn.amount} approved for ${txn.username}` })
  } catch (err) {
    res.status(500).json({ message: 'Server error' })
  }
})
...
router.post('/withdrawals/:id/reject', async (req, res) => {
  try {
    ...
    await User.findByIdAndUpdate(txn.userId, { $inc: { coins: coinsToRefund } })
    ...
    await Transaction.create({
      userId: txn.userId, username: txn.username,
      type: 'bonus', amount: 0, coins: coinsToRefund,
      note: `Refund for rejected withdrawal`,
    })

    await log(req.user.id, req.user.username, 'reject_withdrawal', txn.userId, txn.username,
      `$${txn.amount} rejected: ${reason}`)
    res.json({ message: `Withdrawal rejected and ${coinsToRefund} coins refunded to ${txn.username}` })
  } catch (err) {
    res.status(500).json({ message: 'Server error' })
  }
})
...
router.get('/suspicious', async (req, res) => {
  try {
    const users = await User.find({ isFlagged: true, role: 'player' }).select('-password')
    res.json(users)
  } catch (err) {
    res.status(500).json({ message: 'Server error' })
  }
