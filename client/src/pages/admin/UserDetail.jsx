...
export default function UserDetail() {
  ...
  const addCoins = async () => {
    if (!coins) return
    await api.post(`/api/admin/users/${id}/add-coins`, { coins: Number(coins), reason })
  ...
  const resetBalance = async () => {
    ...
    if (!amt) return
    await api.post(`/api/admin/users/${id}/reset-balance`, { coins: Number(amt) })
  ...
  return (
    <div style={page}>
      <div style={{  margin: '0', display: 'flex', flexDirection: 'column', gap: 16 }}>
        <button style={{ ...btn.ghost, alignSelf: 'flex-start' }} onClick={() => navigate('/admin/users')}>
          ← Back
        </button>
        ...
        <div style={{ ...card, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
          <div>
            <div style={{ fontSize: '1.2rem', fontWeight: 700, color: '#f0d080' }}>{user.username}</div>
            ...
            {user.isBanned && (
              <div style={{ color: '#ff6655', fontSize: '0.7rem', marginTop: 4 }}>
                🚫 BANNED — {user.bannedReason}
              </div>
            )}
          </div>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            <button style={btn.primary} onClick={resetBalance}>Reset Balance</button>
            {user.isBanned
              ? <button style={btn.success} onClick={unban}>Unban</button>
              : <button style={btn.danger}  onClick={ban}>Ban User</button>
            }
          </div>
        </div>
        ...
        {/* Add coins */}
        <div style={card}>
          <div style={{ fontSize: '0.65rem', letterSpacing: '0.2em', color: '#a08050', marginBottom: 12 }}>
            ➕ ADD COINS MANUALLY
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            <input style={{ ...input, marginBottom: 0, flex: 1 }} type="number" placeholder="Coins to add"
              value={coins} onChange={e => setCoins(e.target.value)} />
            <input style={{ ...input, marginBottom: 0, flex: 2 }} placeholder="Reason (optional)"
              value={reason} onChange={e => setReason(e.target.value)} />
            <button style={{ ...btn.primary, whiteSpace: 'nowrap' }} onClick={addCoins}>Add</button>
          </div>
        </div>
        ...
        <div style={card}>
          ...
          <table style={table.wrap}>
            <thead>
              <tr>{['Dice','Total','Bet','Wager','Result','Date'].map(h => <th key={h} style={table.th}>{h}</th>)}</tr>
            </thead>
            <tbody>
              {rounds.map(r => (
                <tr key={r._id}>
                  ...
                  <td style={{ ...table.td, textTransform: 'capitalize', fontSize: '0.65rem' }}>{r.betType}</td>
                  <td style={table.td}>🪙 {r.betAmount}</td>
                  <td style={{ ...table.td, color: r.won ? '#27ae60' : '#c0392b', fontWeight: 700 }}>
                    {r.won ? `+${r.payout}` : `−${r.betAmount}`}
                  </td>
                  <td style={{ ...table.td, fontSize: '0.6rem' }}>{new Date(r.createdAt).toLocaleString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div style={card}>
          ...
          <table style={table.wrap}>
            <thead>
              <tr>{['Type','Coins','Amount','Note','Date'].map(h => <th key={h} style={table.th}>{h}</th>)}</tr>
            </thead>
            <tbody>
              {transactions.map(t => (
                <tr key={t._id}>
                  <td style={{ ...table.td, textTransform: 'uppercase', fontSize: '0.6rem', fontWeight: 700,
                    color: { deposit:'#27ae60', withdraw:'#c0392b', bet:'#a08050', win:'#c9a84c', bonus:'#4488ff' }[t.type] }}>
                    {t.type}
                  </td>
                  ...
                  <td style={{ ...table.td, fontSize: '0.6rem' }}>{new Date(t.createdAt).toLocaleString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
